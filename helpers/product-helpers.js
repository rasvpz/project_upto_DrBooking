...
module.exports = {
  ...
  addNewUser: (userRegistration, session) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      db.get()
        ...
        .findOne({ whatsapp: whatsapp })
        .then((data) => {
          if (!data) {
            db.get()
              .collection("bigOUserRegistration")
              .insertOne(userRegistration)
              .then((data) => {
                resolve(data);
              });
          } else {
    ...
    return new Promise(async(resolve, reject)=>{
        await db.get().collection('bigOUserRegistration').findOne({email:userData.email, cognitoId:session.sub}).then(async(loginData)=>{
           ...
           if(mycDate < 10){  
            ...
            console.log('*******************_________________________', myDate);

           if(loginData){
            var bookings =  await db.get().collection ("bigOUserBookingDetails").find({cognitoId:session.sub, bookingDate:myDate}).toArray ();
            console.log('*******************_________________________', bookings);
            await db.get().collection('bogOclinicsInPanchayath').find({pachayath:panchayath, status:'Running'}).toArray().then(async(myClinic)=>{
                ...
                var data = await db.get().collection('bigOUserBookingDetails').findOne({whatsapp:whatsapp, cognitoId: session.sub})
                var data2 = await db.get().collection('bigOUserBookingDetails').find().sort({_id:-1}).limit(1).toArray()
                var result = await db.get().collection('bigOprescription').findOne({patientCogId: session.sub, status:'processing'})
                var dateFetchingForCompare = await db.get().collection("bigOUserBookingDetails").find({ cognitoId: session.sub, status: 'Booked'}).toArray();
                resolve({myClinic, bookings, data2, data, result,myDate, dateFetchingForCompare})   
            })          
  ...
  verifyPayment: (details, whatsapp, cognitoId) => {
    return new Promise(async (resolve, reject) => {
      ...
      if (checkItsExist == 1) {
        ...
        var currentBookingTime = lastBooking[0].consultingTime;
        var elseBookingTime = lastBooking[0].bookingTime;
        let lastNotBooking = await db
          .get()
          .collection("bigOUserBookingDetails")
          .findOne({ status: "Not-Confirmed" });
  ...
  bookedSuccessfully: (userData, sessionSub) => {
    return new Promise(async (resolve, reject) => {
      let data = await db
        .get()
        .collection("bigOUserBookingDetails")
        .findOne({ whatsapp: userData, cognitoId: sessionSub });
      let data2 = await db
        .get()
        .collection("bigOUserBookingDetails")
        .find()
  ...
  prescribedDrugs: (res) => {
    return new Promise(async (resolve, reject) => {
      let data = await db
        ...
        .collection("bigOUserBookingDetails")
        .updateOne(
          { bookingTime: res.bTime, cognitoId: res.cognitoId },
          { $set: { consultingTime: res.bTime + 01 } }
        );
      resolve(data);
    });
  },
  ...
  updatePaymentStatus: (id) => {
    return new Promise(async (resolve, reject) => {
      let data = await db
        .get()
        .collection("bigOprescription")
        .findOne({ patientCogId: id, status: "processing" });
      let myId = data._id;
      await db
        .get()
        .collection("bigOprescription")
        .updateOne({ _id: ObjectId(myId) }, { $set: { status: "Paid" } })
        .then(() => {
          console.log("sucess===========");
          resolve();
        });
